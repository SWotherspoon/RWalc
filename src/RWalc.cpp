...
Type objective_function<Type>::operator() ()
{
  ...
  vector<Type> sigmabeta2 = exp(Type(2.0)*logSigmaBeta);

  // Backtransform parameters
  vector<Type> sigma = exp(logSigmaBeta+logBeta);
  ...
  for(int i=0; i<dt.size(); ++i) {
    if(seg(i+1)==seg(i)) {
      ...
      Q(0,0) = sigmabeta2(0)*(dt(i)-2.0*(1.0-exp(-beta(0)*dt(i)))/beta(0)+(1.0-exp(-2.0*beta(0)*dt(i)))/(2.0*beta(0)));
      Q(1,1) = sigmabeta2(0)*beta(0)*(1.0-exp(-2.0*beta(0)*dt(i)))/2.0;
      Q(1,0) = sigmabeta2(0)*(1.0-2.0*exp(-beta(0)*dt(i))+exp(-2.0*beta(0)*dt(i)))/2.0;
      Q(0,1) = Q(1,0);

      Q(2,2) = sigmabeta2(1)*(dt(i)-2.0*(1.0-exp(-beta(1)*dt(i)))/beta(1)+(1.0-exp(-2.0*beta(1)*dt(i)))/(2.0*beta(1)));
      Q(3,3) = sigmabeta2(1)*beta(1)*(1.0-exp(-2.0*beta(1)*dt(i)))/2.0;
      Q(2,3) = sigmabeta2(1)*(1.0-2.0*exp(-beta(1)*dt(i))+exp(-2.0*beta(1)*dt(i)))/2.0;
      Q(3,2) = Q(2,3);
